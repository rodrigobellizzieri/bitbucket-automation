...
**`Documentation/example/bitbucket-pipelines.yml`** — the pipeline file that users copy into their central repository. It defines the `bitbucket-automation` custom pipeline, maps UI variables (including `PRD_BRANCHE`) to the Docker pipe, and references the published image by version tag.

**`Documentation/example/templates/`** — example template directories. The `TEMPLATE` variable must exactly match a subdirectory name under `templates/` in the central repo at runtime.
...
- `PRIVATE` accepts only `"private"` or `"public"` (lowercased at runtime); any other value causes `createRepository` to silently skip the API call.
